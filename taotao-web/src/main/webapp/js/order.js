...
//var orderUrl = OrderAppConfig.Domain + "/order/getOrderInfo.html";
var orderUrl = OrderAppConfig.Domain + "/order/getOrderInfo.html";
...
function goOrder() {
    if(isLipinkaPhysical()){
        window.location.href =  OrderAppConfig.Domain + "/order/getLipinkaPhysicalOrderInfo.html?rid=" + Math.random();
    }else{
...
function edit_Consignee(selectId) {
	...
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/editConsignee.html";
...
function loadAllAreaName(id) {
	...
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/loadAreaName.html";
...
function loadAreaDetail(id, provinceId, cityId, countyId, townId) {
	var param = "consigneeParam.provinceId=" + provinceId
			+ "&consigneeParam.cityId=" + cityId + "&consigneeParam.countyId="
			+ countyId + "&consigneeParam.townId=" + townId;
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/loadAllAreas.html";
	jQuery.ajax( {
		type : "POST",
		dataType : "text",
		url : actionUrl,
...
function loadProvinces() {
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/getProvinces.html";
	jQuery.ajax( {
		type : "POST",
		dataType : "text",
		url : actionUrl,
...
function loadCitys() {
	...
	var param = "consigneeParam.provinceId=" + provinceId;
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/getCitys.html";
	jQuery
			.ajax( {
				type : "POST",
				dataType : "text",
...
function loadCountys() {
	...
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/getCountys.html";
	jQuery.ajax( {
		type : "POST",
		dataType : "text",
		url : actionUrl,
...
function loadTowns() {
	...
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/getTowns.html";
	jQuery.ajax( {
		type : "POST",
		dataType : "text",
		url : actionUrl,
...
function delete_Consignee(id) {
	...
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/deleteConsignee.html";
	param = addFlowTypeParam(param);
	jQuery.ajax( {
				type : "POST",
				dataType : "json",
...
function open_easyBuyConsignee(id){
	...
	if(isHidden && (consignee_type == 0 || consignee_type == "0") && consignee_townId <= 0 ){
		...
		var actionUrl =  OrderAppConfig.DynamicDomain + "/consignee/openEasyBuy.html";
		jQuery.ajax( {
			type : "POST",
			dataType : "json",
			url : actionUrl,
...
function save_Consignee() {
	...
	param = addFlowTypeParam(param);
	var actionUrl = OrderAppConfig.DynamicDomain + "/consignee/saveConsignee.html";
...
function edit_Invoice() {
	...
	var actionUrl = OrderAppConfig.DynamicDomain + "/invoice/editInvoice.html";
	// 异步请求获得发票信息
...
function show_GenenalInvoiceDetail(id,isHide){
	var actionUrl = OrderAppConfig.DynamicDomain + "/invoice/getGenenalInvoice.html";
...
function show_ElectroInvoiceDetail() {
	var actionUrl = OrderAppConfig.DynamicDomain + "/invoice/getElectroInvoice.html";
	var invokeInvoiceBasicService =  $("#invokeInvoiceBasicService").val();
	var param =  "invokeInvoiceBasicService="+invokeInvoiceBasicService;
	param = addFlowTypeParam(param);
	jQuery.ajax( {
...
function show_VATInvoiceDetail() {
	var actionUrl = OrderAppConfig.DynamicDomain + "/invoice/getVatInvoice.html";
	var invokeInvoiceBasicService =  $("#invokeInvoiceBasicService").val();
	var param =  "invokeInvoiceBasicService="+invokeInvoiceBasicService;
	param = addFlowTypeParam(param);
	jQuery.ajax( {
...
function show_InvoiceDetail(id,isHide) {
	var actionUrl = OrderAppConfig.DynamicDomain + "/invoice/getNormalInvoice.html";
	var invokeInvoiceBasicService =  $("#invokeInvoiceBasicService").val();
	...
	if (isUseNewInvoice) {
		if(invoice_type ==1 && !(invoice_book_content<=0 && invoice_common_content<=0)){
			...
			jQuery.ajax( {
				type : "POST",
				dataType : "text",
				url : OrderAppConfig.DynamicDomain + "/invoice/addInvoiceToUsual.html",
				data : param,
				cache : false,
				success : function(dataResult, textStatus) {
					usualInvoiceId = 0;
					...
					jQuery.ajax( {
						type : "POST",
						dataType : "text",
						url : OrderAppConfig.DynamicDomain + "/invoice/saveInvoice.html",
						data : param,
						cache : false,
						success : function(dataResult, textStatus) {
							// 没有登录跳登录
...
function delete_Invoice(id) {
	var actionUrl = OrderAppConfig.AsyncDomain
			+ "/invoice/deleteInvoiceFromUsual.html";
	var invokeInvoiceBasicService =  $("#invokeInvoiceBasicService").val();
	var param = "invoiceParam.usualInvoiceId=" + id;
	param = param + "&invokeInvoiceBasicService="+invokeInvoiceBasicService;
	jQuery.ajax( {
...
function edit_Payment(flag) {
	...
	var showPaymentUrl = OrderAppConfig.DynamicDomain
			+ "/payAndShip/showPaymentList.html";
	var param = addFlowTypeParam();
	jQuery.ajax( {
		type : "POST",
		dataType : "text",
...
function edit_Shipment(type) {
	var actionUrl = OrderAppConfig.DynamicDomain
			+ "/payAndShip/getCombinationShipment.html";
...
function showPromise311(paymentId){ 
			jQuery.ajax({
			    ...
			    dataType:"json",
			    async:false,
			    url: OrderAppConfig.DynamicDomain+ "/payAndShip/getPromise311.html?paymentId="+paymentId+addFlowTypeParam('&')+"&rt="+ Math.random(),
...
function showPromise411(paymentId){
	var actionUrl = OrderAppConfig.AsyncDomain+ "/payAndShip/getPromise411.html";
...
//	var actionUrl =  OrderAppConfig.AsyncDomain + "/payAndShip/getPromiseByCurrentOrder.html";
//	var param = addFlowTypeParam();
//	if(resetPromise311){
...
function save_PayAndShip() {
	...
	jQuery.ajax({
		type : "POST",
		dataType : "text",
		url : OrderAppConfig.DynamicDomain + "/payAndShip/savePayAndShip.html",
		data : param,
		cache : false,
		success : function(dataResult, textStatus) {
			// 没有登录跳登录
	...
	jQuery
			.ajax({
				...
				url : OrderAppConfig.DynamicDomain
						+ "/payAndShip/getInstallDates.html?payAndShipParam.bigSkuTimeId="
						+ dateValue,
				data : "",
				cache : false,
				success : function(dataResult, textStatus) {
	...
	if(type != 2){
		...
		$.ajax({
			...
			url : OrderAppConfig.AsyncDomain
					+ "/payAndShip/getShipmentCheckInfo.html",
			data : param,
			cache : false,
			success : function(dataResult, textStatus) {
				// 没有登录跳登录
...
jQuery.ajax({
    ...
    dataType:"json",
    async:false,
    url: OrderAppConfig.DynamicDomain + "/payAndShip/getInstallmentInfo.html?"+addFlowTypeParam()+"&rt="+ Math.random(),
    success:function (data) {
...
function query_coupons() {
	...
	if (flag) {// 显示优惠券列表
		var param = addFlowTypeParam();
		var url = OrderAppConfig.DynamicDomain + "/coupon/getCoupons.html";
		jQuery.ajax( {
			type : "POST",
			dataType : "text",
			url : url,
...
function checkBalancePwdResult(type) {
	...
	param = addFlowTypeParam(param);
	var url = OrderAppConfig.DynamicDomain + "/coupon/checkFundsPwdResult.html";
	jQuery.ajax( {
		type : "POST",
		dataType : "json",
		url : url,
...
function selectJing(obj, key, id) {
	...
	if (flag == 1) {// 选择京券，刷新优惠券列表
		useOrCancelCoupon(OrderAppConfig.DynamicDomain + "/coupon/useCoupon.html", key, obj, 1, jingType);
	} else {
		useOrCancelCoupon(OrderAppConfig.DynamicDomain + "/coupon/cancelCoupon.html", id, obj, 0, jingType);
	}
...
function selectDong(obj, key, id) {
	...
	if (flag == 1) {// 选择东券，刷新优惠券列表
		useOrCancelCoupon(OrderAppConfig.DynamicDomain + "/coupon/useCoupon.html", key, obj, 1, dongType);
	} else {
		useOrCancelCoupon(OrderAppConfig.DynamicDomain + "/coupon/cancelCoupon.html", id, obj, 0, dongType);
	}
}
...
function addEntityCoupon(obj) {
	...
	useOrCancelCoupon(OrderAppConfig.DynamicDomain + "/coupon/useCoupon.html", key,
			obj, 1, "");
}

function removeShiTiCoupon(id) {
	useOrCancelCoupon(OrderAppConfig.DynamicDomain + "/coupon/cancelCoupon.html",
			id, null, 0, "");
}
...
function cancelAllUsedCoupons() {
	...
	var param = addFlowTypeParam();
	var url = OrderAppConfig.DynamicDomain + "/coupon/cancelAllUsedCoupons.html";
	jQuery.ajax( {
				type : "POST",
				dataType : "text",
				url : url,
...
function isNeedPaymentPassword() {
	$("#txt_paypassword").val("");
	var param = addFlowTypeParam();
	var url = OrderAppConfig.DynamicDomain + "/order/isNeedPaymentPassword.html";
	jQuery.ajax( {
		type : "POST",
		dataType : "json",
		url : url,
...
function query_giftCards(giftCardType) {
	...
	if (flag) {// 显示礼品卡列表
		...
		param = addFlowTypeParam(param);
		var url = OrderAppConfig.DynamicDomain + "/giftCard/getGiftCardList.html";
		jQuery.ajax( {
			type : "POST",
			dataType : "text",
			url : url,
...
function selectGiftCard(obj, key, id, giftCardType) {
	...
	if (checked) {
		useOrCancelGiftCard(OrderAppConfig.DynamicDomain
				+ "/giftCard/useGiftCard.html", key, obj, checked, false, giftCardType);
	} else {
		useOrCancelGiftCard(OrderAppConfig.DynamicDomain
				+ "/giftCard/cancelGiftCard.html", id, obj, checked, false, giftCardType);
	}
...
function addGiftCard(obj,giftCardType) {
	...
	useOrCancelGiftCard(OrderAppConfig.DynamicDomain
			+ "/giftCard/useMaterialGiftCard.html", key, obj, false, true, giftCardType);
}
...
function bindGiftCard(key, giftCardType) {
	...
	var url = OrderAppConfig.DynamicDomain + "/giftCard/bindGiftCard.html";
...
function cancelAllUsedGiftCards(giftCardType) {
	...
	var url = OrderAppConfig.DynamicDomain + "/giftCard/cancelAllGiftCard.html";
	jQuery.ajax( {
		type : "POST",
		dataType : "text",
		url : url,
	...
	function useOrCancelBalance(obj) {
		...
		if (flag) {
			url = OrderAppConfig.DynamicDomain + "/balance/useBalance.html";
		} else {
			url = OrderAppConfig.DynamicDomain + "/balance/cancelBalance.html";
		}
...
//		});
	...
	function loadSkuList(){
		var actionUrl = OrderAppConfig.AsyncDomain + "/loadSkuList.html";
		var param = addFlowTypeParam();
       jQuery.ajax( {
	    type : "POST",
	    dataType : "json",
   ...
   error : function(XMLHttpResponse) {
	...
	function loadOrderBalance(){
		if(!$("#selectOrderBalance").is(":checked")){
			var actionUrl = OrderAppConfig.AsyncDomain + "/isShowOrderBalance.html";
			var param = addFlowTypeParam();
			jQuery.ajax( {
				type : "POST",
				dataType : "json",
	...
	function loadCheckCode() {
		var actionUrl = OrderAppConfig.DynamicDomain + "/order/isShowCheckCode.html";
		var param = addFlowTypeParam();
		jQuery.ajax( {
			type : "POST",
			dataType : "json",
	...
	function deleteOutSku(skuIds) {
		...
		var actionUrl = OrderAppConfig.DynamicDomain 
				+ "/order/deleteNoStockSkus.html?orderInfoParam.outSkuIds="
				+ skuIds;
		var param = addFlowTypeParam();
		jQuery.ajax( {
			type : "POST",
	...
	function useOldRepalceNew(){
		...
		jQuery.ajax( {
			type : "POST",
			dataType : "text",
			url : OrderAppConfig.AsyncDomain + "/useOldReplaceNew.html",
	...
	function bigItemChangeArea(){
		var actionUrl=OrderAppConfig.AsyncDomain + "/isBigItemChangeArea.html";
		var param = addFlowTypeParam();
		jQuery.ajax( {
					type : "POST",
					dataType : "json",
	...
	function tang9ChangeArea(){
		var actionUrl=OrderAppConfig.AsyncDomain + "/tang9ChangeArea.html";
		var param = addFlowTypeParam();
		jQuery.ajax( {
			type : "POST",
			dataType : "json",
	...
	function showOrHideJdBean(){
		if($("#orderBeanItem").hasClass("toggle-active")){
			...
			var actionUrl=OrderAppConfig.DynamicDomain + "/jdbean/getJdBean.html";
	...
	function cancelJdBeanWithoutOpen(){
		var actionUrl=OrderAppConfig.DynamicDomain + "/jdbean/useJdBean.html";
	...
	function sendMobileCode(){
		...
		jQuery.ajax({
	        type: "POST",
	        dataType: "json",
	        url: OrderAppConfig.DynamicDomain + "/order/sendMobileCode.html",
	...
	function checkMobileCode(){
		...
		jQuery.ajax({
	        type: "POST",
	        dataType: "json",
	        url: OrderAppConfig.DynamicDomain + "/order/checkMobileCode.html",
	...
	function save_ConsigneeWithoutRegister(){
	    ...
	    jQuery.ajax({
	        type: "POST",
	        dataType: "json",
	        url: OrderAppConfig.DynamicDomain + "/order/saveConsigneeWithoutRegist.html",
	        data: param,
	        cache: false,
	        success: function(dataResult) {
	    		if(dataResult == null){
	...
	function getSkuListWithUuid(){
		jQuery.ajax({
	        type: "POST",
	        dataType: "json",
	        url: OrderAppConfig.DynamicDomain + "/order/getSkuList.html",
	        data: "",
	        cache: false,
	        success: function(dataResult) {
	    		$("#span-skulist").html(dataResult);
...
function changeCodDate(codDateOffset,isJdOrOther){
	...
	if($('#jd-bigItem-install-date').length>0){
		var actionUrl=OrderAppConfig.DynamicDomain + "/payAndShip/getInstallDateList.html";
		if(codDateOffset==-1){
...
function getPickSiteListByRegion(obj){
	...
	jQuery.ajax({
		...
		dataType : "text",
		url : OrderAppConfig.AsyncDomain
				+ "/payAndShip/getPickSiteByRegion.html",
		data : param,
		cache : false,
		success : function(dataResult, textStatus) {
			// 没有登录跳登录
			...
			if(dataResult=="null"){
				...
				window.location.href="http://trade.jd.com/order/getOrderInfo.html?rid=" + Math.random(); 
			}
...
function open_MorePicksite(obj){
	...
	jQuery.ajax({
		...
		dataType : "text", 
		url : OrderAppConfig.DynamicDomain
				+ "/payAndShip/getMorePickSite.html",
		data : param,
		cache : false,
		success : function(dataResult, textStatus) {
			if (isUserNotLogin(dataResult)) {
...
function showPickDateList(obj){
	...
	jQuery.ajax({
		...
		dataType : "text",
		url : OrderAppConfig.AsyncDomain
				+ "/payAndShip/getPickSiteDate.html",
		data : param,
		cache : false,
		success : function(dataResult, textStatus) {
			// 没有登录跳登录
...
//	jQuery.ajax( {
//		type : "POST",
//		url : OrderAppConfig.AsyncDomain + "/showFerightInsure.html",
//		data : param,
//		cache : false,
//		dataType: "json",
//		success : function(dataResult) {
...
function showFerightSopName(){
    if($("#transport").html()!=null){
		...
		jQuery.ajax( {
			type : "POST",
			url : OrderAppConfig.AsyncDomain + "/showFerightSopName.html",
			data : param,
			cache : false,
			dataType: "json",
			success : function(dataResult) {
...
function checkShowOpenPwd(showOpenPayPwd,existsJdbeanPromotion,checkJdbeanPromotion){
	if(existsJdbeanPromotion == true) {
		if(showOpenPayPwd==false){
			...
			$("#submit_message").html("<span>为保障您的账户资金安全，京豆暂时不可用，请先<a href='http://safe.jd.com/user/paymentpassword/safetyCenter.html' target='_blank'>开启支付密码 </a></span> ").show();	
		}
	}
}
...
//	jQuery.ajax({
//		type : "POST",
//		url : OrderAppConfig.AsyncDomain + "/showTangJiuSkuIcon.html",
//		data : param,
//		cache : false,
//		dataType: "json",
//		success : function(dataResult) {
...
function exchangeCoupons(obj) {
	...
	jQuery.ajax( {
				type : "POST",
				dataType : "json",
				url : OrderAppConfig.AsyncDomain + "/coupon/exchangeCoupons.html",
				data : param,
				async : true,
				cache : false,
				success : function(result) {
