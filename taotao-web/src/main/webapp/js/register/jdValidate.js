...
function checkPin(option) {
    ...
    if (!namestate || nameold != pin) {
        if (nameold != pin) {
            ...
            $.ajax({
            	//url : "http://sso.mytaotao.com/user/check/"+escape(pin)+"/1?r=" + Math.random(),
            	url : "http://localhost:8083/user/check/"+escape(pin)+"/1?r=" + Math.random(),
            	dataType : "jsonp",
...
function sendMobileCode() {
    ...
    // 检测手机号码是否存在
    $.ajax({
    	url : "http://localhost:8083/user/check/"+mobile+"/2?r=" + Math.random(),
    	dataType : "jsonp",
